module BaseScore
  ...
  module ClassMethods
    ...
    def best_1_0c(leaderboard_id, user_id, countrycode)
      best_cond = ["leaderboard_id = ? AND user_id = ?", leaderboard_id, user_id]
      best_score = where(best_cond).order("sort_value DESC").limit(1)[0]
      if best_score
		if ENABLE_USER_RANK 
			rank_cond = ["countryCode = ? AND leaderboard_id = ? AND sort_value > ?", countrycode, leaderboard_id, best_score.sort_value]
          sanitized_rank_cond = ActiveRecord::Base.send(:sanitize_sql_array, rank_cond)		  
          best_score.rank = connection.execute("select count(*) from (select user_id from #{table_name}, users where user_id = users.id AND #{sanitized_rank_cond} group by user_id) t").first[0] + 1
        end
      end
      best_score
  end	
	
    def bests_1_0(leaderboard_id, opts = {})
	...
	def bests_1_0_country(leaderboard_id, countrycode, opts = {})
      page_num     = (opts[:page_num] && opts[:page_num].to_i) || 1
      num_per_page = (opts[:num_per_page] && opts[:num_per_page].to_i) || 25
      num_per_page > 25 && num_per_page = 25
      leaderboard_cond = ActiveRecord::Base.send(:sanitize_sql_array, ["leaderboard_id = ?", leaderboard_id])
	  country_cond = ActiveRecord::Base.send(:sanitize_sql_array, ["countryCode = ?", countrycode])
	  
      query =<<-END
      select #{table_name}.* from (
        select t1.*, min(created_at) as first_time from (
          select leaderboard_id, user_id, max(sort_value) as max_val from #{table_name}
		  join users on user_id = users.id and #{country_cond}
          where #{leaderboard_cond}
          group by user_id
          order by max_val DESC
          limit #{num_per_page.to_i}
